...
import 'routes/player_report/player_report.dart';
...
Map<String, MaterialPageRoute> routes(RouteSettings settings) {
 	return {
		...
		_route + '/report': MaterialPageRoute(
			builder: (_) =>  PlayerReport(playerReport: (settings.arguments as List)[0], updateUpperPage: (settings.arguments as List)[1]),
			settings: settings
		)
	};
}
